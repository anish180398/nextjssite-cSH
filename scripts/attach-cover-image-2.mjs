...
// it, and links it as the coverImage on the fine-tuning blog entry.
import { readFileSync } from "node:fs";

const SPACE_ID = process.env.NEXT_PUBLIC_CONTENTFUL_SPACE_ID;
const TOKEN = process.env.CONTENTFUL_MANAGEMENT_TOKEN;
const ENVIRONMENT = "master";
const ENTRY_ID = "6vGQqbUCJdlWcmG8IhRMMx";
const IMAGE_PATH = "/tmp/kryttr-cover-fine-tuning-llm.png";
...
async function main() {
  ...
  const assetRes = await fetch(
    `https://api.contentful.com/spaces/${SPACE_ID}/environments/${ENVIRONMENT}/assets`,
    {
      ...
      body: JSON.stringify({
        fields: {
          title: { "en-US": "Fine-tuning an LLM \u2014 calibration illustration" },
          file: {
            "en-US": {
              contentType: "image/png",
              fileName: "fine-tuning-llm-calibration.png",
